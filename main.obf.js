...
(() => {
  ...
  function applyAppearanceFilter() {
    ...
    // vibrance is simulated via saturate with a slight boost factor
    ...
    // Apply to canvas / game wrapper but NOT to the UI root
  ...
  function joinerHandleTeam(team) {
    ...
    // Always observe DOM for button to appear or become enabled
...
/* ── Cool battle result / killboard element styling ── */
  ...
  function Threads() {
    ...
    const candidates = getFunction(
      arr,
      "BattleEntity.createComponent\x20duplicate\x20component\x20class\x20",
      2,
      true,
    );
  ...
  // Removes layered (stacked) mines: keeps the first mine at each Vector3
  // position and deactivates duplicates.
  // ally === true  -> remove friendly stacked mines
  // ally === false -> remove enemy stacked mines
  function Decluster(ally) {
    const mode = GameMode();
    if (!mode) return;
    const myTeam = getCurrentTeam(LocalTank());
    if (!myTeam) return;
    const root = getFunction(mode, "putInitialMines");
    ...
    for (const m of maps) {
      ...
      for (const k in internal) {
        const bucket = internal[k];
        if (!bucket || typeof bucket !== "object") continue;
        const isArr = Array.isArray(bucket);
        const isView = ArrayBuffer.isView(bucket);
        if (isArr && bucket.some((v) => typeof v === "number")) continue;
        if (!isArr && isView) continue;

        for (const ent of bucket) {
          if (!ent || typeof ent !== "object") continue;
          const compHolder = getFunction(
            ent,
            "BattleEntity.createComponent duplicate component class ",
          );
          if (!compHolder) continue;

          let tank = null;
          for (const ck in compHolder) {
            const v = compHolder[ck];
          ...
          const team = getCurrentTeam(tank);
          if (!team) continue;
          const isAlly = team === myTeam;
          if ((ally && !isAlly) || (!ally && isAlly)) continue;

          const stateComp = getFunction(tank, "stateComponent");
          if (!stateComp) continue;
          const vec = getFunction(stateComp, "Vector3");
          ...
          if (seen.has(posKey)) {
            const eqHolder = getFunction(compHolder, " = ");
            ...
            for (const layer of inner.values()) {
              let arr = null;
              for (const lk in layer) {
                const v = layer[lk];
                if (v && Array.isArray(Object.values(v)?.[0])) {
                  arr = Object.values(v)[0];
                  break;
                }
              }
              if (!arr) continue;
...
function renderListTab() {
  ...
  function drawBackgroundHolography() {
    if (bgState.currentMode !== 1) { bgState.holoRunning = false; return; }
    const canvas = bgState.holoCanvas;
    if (!canvas) return;
  ...
  function drawBackgroundSpace() {
    if (bgState.currentMode !== 2) { bgState.spaceRunning = false; return; }
    const canvas = bgState.spaceCanvas;
    if (!canvas) return;
    const context = canvas.getContext("2d");
    context.clearRect(0, 0, canvas.width, canvas.height);
